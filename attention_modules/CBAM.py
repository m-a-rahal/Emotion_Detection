...
def channel_attention_mask(tensor, ratio=8):
    ...
    assert c >= ratio, f"there should be at least {ratio} channels in the input of channel_attention_mask (equal to ratio)"
...
def spacial_attention_mask(tensor, kernel_shape=None):
    """ the paper Woo et al. recommended a 7x7 kernel_shape,
    ...
    mask_conv = Conv2D(1, kernel_shape, padding='same', activation='sigmoid', kernel_initializer='he_normal',
                       use_bias=False)
...
if __name__ == '__main__':
    class CBAM(object):
        ...
        def next_block(self, tensor, name='CBAM_block', ratio=8, kernel_shape=None):
            block = Model(tensor, cbam_attention(tensor, ratio=ratio, kernel_shape=kernel_shape),
